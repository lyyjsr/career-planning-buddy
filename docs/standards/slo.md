...
| 记忆层贡献（k=1 单次） | +13.3pp | p=0.129 ❌ 不显著 |
| **记忆层贡献（k=3 复跑，`77539891` OFF vs `9933d100` ON）** | **+4.3pp**（82.2% vs 86.5%） | **p=0.429 ❌ 不显著——k=1 的 +13.3pp 系单次方差膨胀，按预注册制度正式撤回** |
...
| 删除的可观测退化 | live-mem 0/3（接地失败）、replan 上下文丢失；硬门禁差 +4.3pp（k=3，不显著） |
