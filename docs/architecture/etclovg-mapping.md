...
## 检索管线设计决策记录（2026-08-26）

### 门控相关性边界的定义

**决策**：相对门控阈值定为 5× median（宽松），接受相邻领域经验通过。

**背景**：增强评测（5 个负例）发现门控有 1/5 假阳性——查询"Tableau 可视化"
找到了"Elasticsearch 运维"（比率 38.5×，远超阈值）。分析后判定这不是缺陷
而是产品定位选择：

- 职业规划场景：用户问"会不会 X"，系统找到相邻领域 Y → "你有可迁移基础"
  比"没找到"更有价值 → 宽松门控（5×）
- 严格问答场景：要求精确匹配否则拒答 → 紧门控（50×）

**实测数据**：
  正例（15 case）：比率 227×~680× → 全部正确通过
  负例（5 case）：比率 2.2×~3.9× → 4/5 正确拒绝
  边界（1 case）：比率 38.5× → 通过（相邻领域，设计意图内）

### 重排的 trade-off 量化

| 模式 | P50 延迟 | nDCG@5 | 结论 |
|---|---|---|---|
| hybrid（不加重排） | 8ms | 0.910 | 当前规模最优 |
| hybrid + GPU 重排 | 400ms | 0.917 | 50 倍延迟换 0.8% 提升 |

**结论**：小语料（<100 chunk）不加重排；大语料（1000+）再启用。重排架构
（retrieve wide → rerank narrow + 相对门控 + 缓存）已就绪，配置切换即可。

### 词法通道的优势发现

pg_trgm 词法检索的 MRR@1（86.7%）高于向量检索（73.3%）——转述查询中
特定领域术语的精确字符匹配有时比语义嵌入更准。这解释了 hybrid 融合的
价值：两通道覆盖不同失败模式。
