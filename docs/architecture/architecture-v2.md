...
| 臂 | 配置 | 硬门禁 | live-mem 接地 |
|---|---|---|---|
| 假裸模型（`8d3f4781`） | direct 生成器，**但仍享图骨架：预执行 memory_lookup + 上下文记忆注入** | 93.3% | 3/3 |
| **真裸模型（`6b03e9af`）** | MEMORY_DISABLED + direct 生成器，无记忆无工具 | **72.4%**（21/29） | **0/3** |
| 完整 Agent（`85d6ba48`/`cd3eb74e`） | 全架构 | 93.3%（k=1）/ 88.9%（k=3） | 3/3 |

第一轮对照（93.3% vs 83.3%）曾被解读为"裸模型赢过 Agent"——**E1 复核发现
该"裸"臂并非真裸**：direct 变体只关掉了模型自主工具调用，确定性预执行与
记忆注入仍在工作。补跑真裸臂后结论反转：**模型裸能力 72.4%，Agent 骨架
（记忆注入 + 预执行 + 校验修复）贡献约 +21pp**；此前的"输"全部来自修复环
自身的降级质量（已修复，k=3 权威口径 88.9%）。原始对照的教训已入档：
...
**恢复能力（E3，实测）**：graceful-shutdown 中断 + 二次执行的对照
（`scripts/crash_recovery_measurement.py`）：attempt 1 在 planning 完成后被
中断释放，attempt 2 从 candidate checkpoint 恢复——**0 次新增物理 LLM 调用**
（恢复步骤 token 归因来自 checkpoint、延迟为 0），朴素全量重跑将把规划
成本翻倍。

**记忆质量盲评（C2，独立 DeepSeek judge，n=8/臂，盲态）**：
D1 目标对齐 2.50→2.62、D2 证据接地 3.00→3.00、D3 可执行 3.38→3.50、
D4 周视界 2.75→2.38。**结论：记忆层对盲评质量分的效应在 n=8 下不显著**。
与 memory_grounded 词面指标（信噪比不足、领域通用词假阳性）结论一致：
**记忆层的可证价值在接地能力（live-mem 0/3 vs 3/3，+10pp 硬门禁），质量
维度的提升需要 prompt 工程迭代（v6 已上线，live-mem 3/3 全过但文本接地
尚无显著改善）**。两条独立证据线同向，构成诚实的完整论证。
